...
*/
export function createComponentConnector<TContext>(
  context: React.Context<TContext>
): IComponentConnector<TContext, {}> {
  function connect<TComponentProps, TSelectedContext>(
      selector: (context: TContext) => TSelectedContext,
      Component: React.FunctionComponent<TComponentProps & TSelectedContext>
  ) {
      const MemoizedComponent = React.memo<React.FunctionComponent<TComponentProps & TSelectedContext>>(Component);
  ...
  const connector = {
      connect: function <TSelectedContext>(
          selector: (context: TContext) => TSelectedContext,
          Component: React.FunctionComponent<TSelectedContext>
      ) {
          return connect(selector, Component);
      },
      connectContextProps: function <TPropertyKeys extends keyof TContext>(
          contextPropertyNames: TPropertyKeys[],
          Component: React.FunctionComponent<Pick<TContext, TPropertyKeys>>
      ) {
...
*/
export function selectProps<TObject extends Object, TPropertyKeys extends keyof TObject>(
  object: TObject,
  propertyNames: Array<TPropertyKeys>
...
*
* @param initialState Initial state of the property bag or a function to compute the initial state.
* @returns [The current property bag state, dispatcher to update parts of the property bag]
*/
export function usePropertyBag<T extends Object>(initialState: T | (() => T)) {
  const reducer = (state: T, action: Partial<T> | ((state: T) => Partial<T>)) => {
      const newState = typeof action === "function" ? action(state) : action;
      return { ...state, ...newState };
  };
  return React.useReducer<React.Reducer<T, Partial<T> | ((state: T) => Partial<T>)>>(
      reducer,
